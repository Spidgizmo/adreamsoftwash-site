alter table public.signup_payment_notification_outbox
  drop constraint if exists signup_payment_notification_outbox_status_check;
alter table public.signup_payment_notification_outbox
  add constraint signup_payment_notification_outbox_status_check
  check (status in ('scheduled','queued','simulated','sent','canceled','failed'));

create or replace function public.prepare_stripe_test_checkout_for_auth(
  p_lead_id uuid,
  p_auth_user_id uuid
)
returns jsonb
...
begin
  if p_auth_user_id is null then
    raise exception 'An authenticated customer identity is required';
  end if;
  ...
  if lead.id is null then raise exception 'Pending signup is not owned by this authenticated identity'; end if;
  if lead.status <> 'submitted_unpaid' then raise exception 'Signup must be submitted and unpaid before checkout recovery'; end if;
...
revoke all on function public.prepare_stripe_test_checkout_for_auth(uuid,uuid) from public,anon,authenticated;
grant execute on function public.prepare_stripe_test_checkout_for_auth(uuid,uuid) to service_role;
