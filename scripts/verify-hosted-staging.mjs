const optionalHostedAuthNames = [
  "NEXT_PUBLIC_SUPABASE_URL",
  "NEXT_PUBLIC_SUPABASE_ANON_KEY",
  "ADS_STAGING_TEST_PASSWORD",
];

const configuredHostedAuthNames = optionalHostedAuthNames.filter((name) =>
  Boolean(process.env[name]?.trim()),
);
if (
  configuredHostedAuthNames.length > 0 &&
  configuredHostedAuthNames.length !== optionalHostedAuthNames.length
) {
  throw new Error(
    "Hosted authentication verification requires NEXT_PUBLIC_SUPABASE_URL, NEXT_PUBLIC_SUPABASE_ANON_KEY, and ADS_STAGING_TEST_PASSWORD together.",
  );
}

const verifyHostedAuthentication =
  configuredHostedAuthNames.length === optionalHostedAuthNames.length;
if (
  verifyHostedAuthentication &&
  process.env.ADS_STAGING_TEST_PASSWORD.length < 16
) {
...
const supabaseUrl = verifyHostedAuthentication
  ? hostedHttpsUrl(
...
const password = process.env.ADS_STAGING_TEST_PASSWORD?.trim() || null;
...
if (verifyHostedAuthentication) {
  ...
  console.log(
    "Hosted authentication/RLS smoke checks were not repeated because protected staging credentials were not supplied to this workflow. The PR database/security workflow still runs on this commit.",
  );
}

console.log(
  `Hosted staging verified at ${appUrl.origin}: Step 3 server-only simulators, redaction, Stripe-disabled boundary, health, and safety banner passed; protected hosted authentication checks ran only when their complete credential set was supplied.`,
);
