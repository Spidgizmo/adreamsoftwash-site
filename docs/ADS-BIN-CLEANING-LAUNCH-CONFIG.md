...
**Configuration version:** `2026-08-04-launch-rules-v1`  
**System:** ADS Bin Cleaning at `www.acleanbin.com`

This is the single owner-approved launch-rule reference for the website, signup, CRM, Stripe configuration, portal, reporting, scheduling, and tests. Code mirrors these rules in `src/lib/bin-cleaning-launch-config.ts`. The service-plan prices remain sourced only from the versioned central plan catalog.
...
## 4. Private card promotion — ONE45

The exact promo code is **`ONE45`**: the letters `O-N-E`, followed by the digits `4-5`. It is not the number `145`.

- Campaign: card-only two-bin one-time special.
- Redemption method: customer enters `ONE45` in the promotional-code field before payment.
- General website price chart: the offer is not displayed there; normal One-Time pricing remains $60 for up to two bins.
- Eligible purchase: One-Time Cleaning with exactly two bins.
- Promotional subtotal: $45 before tax.
- Normal subtotal: $60 before tax.
- Discount: $15.
- Customer eligibility: new ADS Bin Cleaning customer acquisition offer.
- Usage limit: one successful redemption per customer and one successful redemption per service address.
