...
GitHub Actions verified the complete disposable environment on commit `a0a33c418f768667b661043379c49ca49a0f90ac`: `npm test` passed 42/42, typecheck passed, lint passed with two pre-existing image warnings, build passed, `supabase db reset` passed, `supabase db lint --local` reported no schema errors, and `supabase test db` passed 74/74 assertions across six files. The workflow also provisioned all six fictional passwords, signed in every customer/staff test identity through Supabase Auth, and verified the expected customer-row visibility through the live REST/RLS boundary. This verifies the disposable test foundation only; it does not activate or verify a production project.
