...
GitHub Actions verified the complete disposable environment on commit `dd7fcd265b8e7904e3a57b72737628634a433027`: `npm test` passed 42/42, typecheck passed, lint passed with two pre-existing image warnings, build passed, `supabase db reset` passed, `supabase db lint --local` reported no schema errors, and `supabase test db` passed 80/80 assertions across seven files. The workflow also provisioned all six fictional passwords, signed in every customer/staff test identity through Supabase Auth, and verified the expected customer-row visibility through the live REST/RLS boundary. This verifies the disposable test foundation only; it does not activate or verify a production project.
