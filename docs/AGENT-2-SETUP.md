...
GitHub Actions verified the complete disposable environment on commit `be14bd26bb60b1551511d596ce797daae4432fea`: `npm test` passed 42/42, typecheck passed, lint passed with two pre-existing image warnings, build passed, `supabase db reset` passed, `supabase db lint --local` reported no schema errors, and `supabase test db` passed 83/83 assertions across seven files. The workflow also provisioned all six fictional passwords, signed in every customer/staff test identity through Supabase Auth, and verified the expected customer-row visibility through the live REST/RLS boundary. This verifies the disposable test foundation only; it does not activate or verify a production project.
