...
export async function crmCustomers(filters: { q?: string; plan?: string; status?: string; municipality?: string; pickup?: string }) {
  ...
  return rows.filter((customer) => (!filters.plan || customer.subscriptions.some((subscription) => subscription.service_plan_versions.service_plans.id === filters.plan)) && (!filters.municipality || customer.service_addresses.some((address) => address.municipalities?.name === filters.municipality)) && (!filters.pickup || customer.service_addresses.some((address) => address.trash_pickup_schedules.some((pickup) => String(pickup.weekday) === filters.pickup))));
}
...
export async function crmCustomer(id: string) {
  ...
  return (await databaseRequest<CustomerRow[]>(`customers?id=eq.${safe}&select=${customerSelect}&service_addresses.is_current=eq.true&service_addresses.recycling_pickup_schedules.is_current=eq.true&subscriptions.ended_at=is.null&subscriptions.order=started_at.desc.nullslast&subscriptions.limit=1`))[0] ?? null;
}

export type VisitRow = { id: string; status: string; scheduled_for: string | null; cleaning_confirmed: boolean; bins_returned: boolean; customers: { full_name: string }; route_stops: { service_addresses: { line1: string; city: string; preferred_return_location: string | null; access_instructions: string | null; animal_warning: string | null; bins: { id: string; identifier: string | null; description: string | null }[] } } | null; visit_photographs: { id: string; kind: string }[] };
