...
function jsonBody(value: unknown): RequestInit {
  return { method: "PATCH", body: JSON.stringify(value), headers: { Prefer: "return=minimal" } };
}
...
async function syncBins(addressId: string, stream: "trash" | "recycling", desired: number) {
  ...
  if (existing.length > desired) {
    for (const bin of existing.slice(desired)) await databaseRequest(`bins?id=eq.${bin.id}`, jsonBody({ active: false }));
  } else if (existing.length < desired) {
    for (let index = existing.length; index < desired; index += 1) {
      await databaseRequest("bins", { method: "POST", body: JSON.stringify({ service_address_id: addressId, collection_stream: stream, description: `${stream === "trash" ? "Trash" : "Recycling"} bin ${index + 1}`, active: true }), headers: { Prefer: "return=minimal" } });
    }
  }
}
...
export async function POST(request: NextRequest) {
  ...
  if (!fullName || !email || !line1 || !city || !/^[A-Z]{2}$/.test(region) || postalCode.length < 5 || !Number.isInteger(trashBins) || !Number.isInteger(recyclingBins) || trashBins < 0 || recyclingBins < 0 || trashBins + recyclingBins < 1 || trashBins + recyclingBins > 20) {
    return NextResponse.redirect(new URL(`/bin-cleaning/crm/customers/${customerId}?error=validation`, request.url), 303);
  }

  await databaseRequest(`customers?id=eq.${customerId}`, jsonBody({ full_name: fullName, email, phone: phone || null, account_status: accountStatus }));
  const normalized = `${line1}|${line2}|${city}|${region}|${postalCode}`.toLowerCase().replace(/\s+/g, " ").trim();
  ...
  if (Number.isInteger(trashWeekday) && trashWeekday >= 0 && trashWeekday <= 6) {
    const schedules = await databaseRequest<{ id: string }[]>(`trash_pickup_schedules?service_address_id=eq.${addressId}&effective_to=is.null&select=id&limit=1`);
    if (schedules[0]) await databaseRequest(`trash_pickup_schedules?id=eq.${schedules[0].id}`, jsonBody({ weekday: trashWeekday, customer_reported_weekday: trashWeekday, source: "staff_verified", verification_status: "staff_verified" }));
  }

  if (recyclingBins > 0 && Number.isInteger(recyclingWeekday) && recyclingWeekday >= 0 && recyclingWeekday <= 6 && [1, 2].includes(recyclingFrequency) && /^\d{4}-\d{2}-\d{2}$/.test(recyclingAnchor)) {
    const schedules = await databaseRequest<{ id: string }[]>(`recycling_pickup_schedules?service_address_id=eq.${addressId}&is_current=eq.true&select=id&limit=1`);
    if (schedules[0]) await databaseRequest(`recycling_pickup_schedules?id=eq.${schedules[0].id}`, jsonBody({ weekday: recyclingWeekday, frequency_weeks: recyclingFrequency, anchor_collection_date: recyclingAnchor, source: "staff_verified", verification_status: "staff_verified" }));
  }

  const subscriptionId = safeId(text(form, "subscription_id"));
  if (subscriptionId && ["monthly", "quarterly", "twice-yearly", "one-time"].includes(planId)) {
    const versions = await databaseRequest<{ id: string }[]>(`service_plan_versions?plan_id=eq.${encodeURIComponent(planId)}&retired_at=is.null&select=id&order=effective_at.desc&limit=1`);
    if (versions[0]) await databaseRequest(`subscriptions?id=eq.${subscriptionId}`, jsonBody({ service_plan_version_id: versions[0].id }));
  }

  if (staffNote) {
    await databaseRequest("customer_notes", { method: "POST", body: JSON.stringify({ customer_id: customerId, body: staffNote, visibility: "staff_only", created_by: session.id }), headers: { Prefer: "return=minimal" } });
  }
