...
function displayDate(value:string|null){if(!value)return"Never";return new Intl.DateTimeFormat("en-US",{month:"short",day:"numeric",year:"numeric",hour:"numeric",minute:"2-digit",timeZone:"America/New_York"}).format(new Date(value));}
function requestedText(value:Record<string,unknown>|null){if(!value)return"No details supplied";if(typeof value.value==="string")return value.value;return Object.entries(value).filter(([key])=>key!=="submitted_from").map(([key,item])=>`${key.replaceAll("_"," ")}: ${String(item??"—")}`).join(" · ");}

export default async function Customer({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{saved?:string;error?:string;reviewed?:string;erase_error?:string;move_applied?:string;move_error?:string}>}){
 const [{id},query]=await Promise.all([params,searchParams]);
 const customer=await crmCustomer(id);if(!customer)notFound();
 const address=customer.service_addresses[0];
 const [notes,audit,changes,bins,moveInfo]=await Promise.all([
  ...
  databaseRequest<{cancellation_reason:string|null;move_out_date:string|null;service_through_date:string|null}[]>(`customers?id=eq.${customer.id}&select=cancellation_reason,move_out_date,service_through_date`).then(r=>r[0]??{cancellation_reason:null,move_out_date:null,service_through_date:null})
 ]);
 ...
 const trashCount=bins.filter(b=>b.collection_stream==="trash").length;
 const recyclingCount=bins.filter(b=>b.collection_stream==="recycling").length;
 ...
 return <AppShell area="Internal CRM">
  ...
  {query.saved&&<p className="mt-4 rounded-xl bg-emerald-50 p-4 font-bold text-emerald-900">Customer record updated.</p>}
  {query.error&&<p className="mt-4 rounded-xl bg-red-50 p-4 font-bold text-red-900">Customer update needs correction.</p>}
  {query.erase_error&&<p className="mt-4 rounded-xl bg-red-50 p-4 font-bold text-red-900">This customer could not be permanently erased. No partial browser-side deletion was attempted.</p>}
  {query.move_applied&&<p className="mt-4 rounded-xl bg-emerald-50 p-4 font-bold text-emerald-900">Service move approved. The new address is current; the prior address remains in history and future old-address visits were canceled for rerouting.</p>}
  {query.move_error&&<p className="mt-4 rounded-xl bg-red-50 p-4 font-bold text-red-900">The service move was not applied. Confirm the address is serviceable and that the requested move date is today or earlier, then try again.</p>}
  ...
  <div className="mt-6 grid gap-6 lg:grid-cols-3">
   <section className="card p-5 lg:col-span-2">
    <dl className="grid gap-5 sm:grid-cols-2">
     ...
     {moveInfo.cancellation_reason&&<Definition label="Cancellation reason">{moveInfo.cancellation_reason}</Definition>}
     {moveInfo.move_out_date&&<Definition label="Move-out date">{moveInfo.move_out_date}</Definition>}
     {moveInfo.service_through_date&&<Definition label="Service through">{moveInfo.service_through_date}</Definition>}
    </dl>
   </section>
   <section className="card p-5">
    ...
    {pendingChanges.length===0?<p className="mt-2 text-sm text-zinc-500">No pending requests.</p>:pendingChanges.map(c=><div className="mt-4 border-t pt-4" key={c.id}>
      <p className="text-xs font-bold uppercase tracking-wide text-zinc-500">{c.request_type.replaceAll("_"," ")}</p>
      <p className="mt-1 text-sm"><strong>Requested:</strong> {requestedText(c.requested_value)}</p>
      {c.request_type==="service_address_move"&&<div className="mt-3 rounded-lg border border-blue-200 bg-blue-50 p-3">
        <p className="text-xs font-semibold text-blue-950">Confirm the new address is inside the ADS service area and review the pickup schedule before approving. A future-dated move cannot be activated early.</p>
        <form action="/api/bin-cleaning/crm/apply-service-move" method="post" className="mt-3">
          <input type="hidden" name="request_id" value={c.id}/><input type="hidden" name="customer_id" value={customer.id}/>
          <button className="rounded-lg bg-brand-700 px-3 py-2 text-sm font-black text-white">Approve move & make new address current</button>
        </form>
      </div>}
    </div>)}
   </section>
  ...
  <details className="card mt-5 p-5"><summary className="flex cursor-pointer list-none items-center justify-between gap-4"><div><h3 className="text-xl font-black">Customer account & service details</h3><p className="mt-1 text-sm text-zinc-600">Open this section when staff needs to change customer information, service details, move status, or add an internal note.</p></div><span className="rounded-lg border px-3 py-2 text-xs font-black">Open / close</span></summary>
   <form action="/api/bin-cleaning/crm/customer-update" method="post" className="mt-5 grid gap-4 border-t pt-5 sm:grid-cols-2 lg:grid-cols-3">
    <input type="hidden" name="customer_id" value={customer.id}/><input type="hidden" name="address_id" value={address?.id??""}/><input type="hidden" name="subscription_id" value={subscription?.id??""}/>
    ...
    <label className="font-semibold">Cancellation reason<select name="cancellation_reason" defaultValue={moveInfo.cancellation_reason??""} className="mt-1 w-full rounded-lg border p-3"><option value="">Not canceled</option><option value="moved">Moved</option><option value="price">Price</option><option value="service_issue">Service issue</option><option value="no_longer_needed">No longer needed</option><option value="other">Other</option></select></label>
    <label className="font-semibold">Move-out date<input type="date" name="move_out_date" defaultValue={moveInfo.move_out_date??""} className="mt-1 w-full rounded-lg border p-3"/></label>
    <label className="font-semibold">Service through date<input type="date" name="service_through_date" defaultValue={moveInfo.service_through_date??""} className="mt-1 w-full rounded-lg border p-3"/><span className="mt-1 block text-xs font-normal text-zinc-500">Future route visits after this date are removed when the cancellation reason is Moved.</span></label>
    ...
    <label className="font-semibold">Trash pickup<select name="trash_weekday" defaultValue={trashSchedule?.weekday??""} className="mt-1 w-full rounded-lg border p-3"><option value="">Select day</option>{standardPickupDays.map(({day,index})=><option value={index} key={day}>{day}</option>)}</select></label>
    <label className="font-semibold">Recycling pickup<select name="recycling_weekday" defaultValue={recyclingSchedule?.weekday??""} className="mt-1 w-full rounded-lg border p-3"><option value="">Select day</option>{standardPickupDays.map(({day,index})=><option value={index} key={day}>{day}</option>)}</select></label>
    ...
    <label className="font-semibold lg:col-span-3">Add staff-only note<textarea name="staff_note" className="mt-1 w-full rounded-lg border p-3"/></label>
