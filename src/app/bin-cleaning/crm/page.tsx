        ...
        <Stat
          ...
          value={customers.filter((customer) =>
            customer.service_addresses.some((address) =>
              address.trash_pickup_schedules.some((pickup) => pickup.weekday == null),
            ),
          ).length}
        />
        <Stat
      ...
      <section className="card mt-6 overflow-hidden">
        <div className="border-b p-5">
          ...
          <p className="mt-1 text-sm text-zinc-600">
            Compact intake list. Click a customer name to open every saved signup field.
            No signup shown here is paid or active.
          </p>
        </div>
        ...
        {!signupResult.available ? (
          <div className="m-5 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm font-bold text-amber-950">
            The Step 4 signup table is not available in this hosted staging database yet.
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[980px] text-left text-sm">
              <thead>
                <tr>
                  {["Customer", "Status", "Plan", "Bins", "Pickup", "Discount", "Estimate", "Last activity"].map((heading) => (
              ...
              <tbody>
                {signupLeads.map((lead) => (
                  <tr key={lead.id} className="border-t align-middle hover:bg-zinc-50">
                    <td className="p-3">
                      <Link
                        className="font-black text-brand-700 underline-offset-2 hover:underline"
                        href={`/bin-cleaning/crm/signups/${lead.id}`}
                      >
                        {lead.full_name || "Incomplete name"}
                      </Link>
                      <p className="mt-1 text-xs text-zinc-500">{lead.email || "No email yet"}</p>
                    </td>
                    <td className="p-3">
                      <span className={`inline-flex rounded-full px-2 py-1 text-xs font-black uppercase tracking-wide ${lead.status === "submitted_unpaid" ? "bg-emerald-100 text-emerald-900" : lead.status === "abandoned" ? "bg-amber-100 text-amber-950" : "bg-blue-100 text-blue-900"}`}>
                        {lead.status.replaceAll("_", " ")}
                      </span>
                    </td>
                    <td className="p-3 capitalize">{lead.plan_id || "—"}</td>
                    <td className="p-3">
                      <strong>{lead.bin_count}</strong>
                      <span className="ml-1 text-xs text-zinc-500">
                        ({lead.bin_streams?.trash ?? 0}T/{lead.bin_streams?.recycling ?? 0}R/{lead.bin_streams?.other ?? 0}O)
                      </span>
                    </td>
                    <td className="p-3">{lead.trash_weekday == null ? "—" : days[lead.trash_weekday]}</td>
                    <td className="p-3">{signupDiscount(lead)}</td>
                    <td className="p-3 font-bold">
                      {lead.estimated_first_charge_cents == null ? "Pending" : `${formatCurrency(lead.estimated_first_charge_cents)} before tax`}
                    </td>
      ...
      <section className="card mt-6 overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[800px] text-left text-sm">
            <thead>
              <tr>
                {["Customer", "Status", "Plan", "Municipality", "Pickup", "Cleaning"].map((heading) => <th className="p-3" key={heading}>{heading}</th>)}
              </tr>
            </thead>
            <tbody>
              {customers.map((customer) => {
                ...
                return (
                  <tr key={customer.id} className="border-t">
                    <td className="p-3"><Link href={`/bin-cleaning/crm/customers/${customer.id}`}>{customer.full_name}</Link></td>
                    <td>{customer.account_status}</td>
                    <td>{customer.subscriptions[0]?.service_plan_versions.service_plans.display_name}</td>
                    <td>{address?.municipalities?.name}</td>
                    <td>{pickup?.weekday == null ? "Unverified" : days[pickup.weekday]}</td>
                    <td>{pickup?.cleaning_day_assignments[0] ? days[pickup.cleaning_day_assignments[0].normal_weekday] : "Pending"}</td>
                  </tr>
                );
              })}
            </tbody>
